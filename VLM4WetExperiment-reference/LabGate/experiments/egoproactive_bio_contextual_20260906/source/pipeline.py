...
def generate_with_history(model, frames, prompt, history):
    """Keep old model/fake-model interfaces working when no history is supplied."""
    if history is None:
        return model.generate(frames, prompt)
    return model.generate(frames, prompt, history=history)
...
class GateResult:
    ...
    judger_prompt: str
    expert_prompt: str
    history_sent: list[dict[str, str]]
    current_step_sent: str
...
class LabGate:
    ...
    def __init__(
        ...
        judger: QwenVL,
        expert: QwenVL,
        tau_reproj: float = 0.12,
        proactive_next_step: bool = False,
    ):
        self.judger = judger
        self.expert = expert
        self.tau_reproj = tau_reproj
        self.proactive_next_step = proactive_next_step
    ...
    def run_frames(
        ...
        if always_expert:
            ...
            j_prompt = judger_prompt(
                protocol,
                asr_text,
                proactive_next_step=self.proactive_next_step,
                current_step=current_step,
            )
            j_reply, dt_j = generate_with_history(
                self.judger, kept, j_prompt, history
            )
        ...
        e_prompt = ""
        if fire:
            e_prompt = expert_prompt(
                protocol,
                asr_text,
                judger_reason=reason,
                current_step=current_step,
            )
            e_reply, dt_e = generate_with_history(
                self.expert, kept, e_prompt, history
            )
        ...
        return GateResult(
            ...
            judger_prompt=j_prompt,
            expert_prompt=e_prompt,
            history_sent=list(history or []),
            current_step_sent=current_step,
        )
